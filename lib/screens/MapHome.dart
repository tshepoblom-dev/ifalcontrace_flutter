...
class _MapPageState extends State<MapPage> {
  ...
  //  Uint8List data = Uint8List.view(pngBytes.buffer ?? 0);
    Uint8List data;
    if (pngBytes != null) {
      data = Uint8List.view(pngBytes.buffer);
    } else {
      // Handle the case where pngBytes is null, e.g., assign an empty Uint8List.
      data = Uint8List(0);
    }
  ...
  void updateMarkerInfo(int deviceId, var iconPath, controller) {
    ...
    if(controller.positions[_selectedDeviceId]!.attributes!.containsKey("idleDuration")){
        //idleDurationValue = 
        //prettyDuration(controller.positions[_selectedDeviceId].attributes["idleDuration"]);
        var idleDuration = controller.positions[_selectedDeviceId].attributes["idleDuration"];
      if (idleDuration != null) {
        idleDurationValue = idleDuration;
      } else {
        // Handle the null case, e.g., set a default value
        idleDurationValue = "Not yet available"; // Or any other default value or message
      }
    }
      bool blocked = false;
            ...
            child: Column(
              children: <Widget>[
                ...
                FloatingActionButton(
                  heroTag: "reloadMap",
                  mini: true,
                  onPressed: (){_reloadMap(controller);},
