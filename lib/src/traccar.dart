...
class Traccar {
  ...
  static Future<List<RouteReport>?> getRoute(
    ...
    final response = await http.get(
        Uri.parse(serverURL! +
            ...
            "&to=" +
            to),
        headers: headers).timeout(Duration(seconds: 120));
        
    var _url = Uri.parse(serverURL! +
       "/api/reports/route?deviceId=" +
       deviceId +
       "&from=" +
       from +
  ...
  static Future<List<Event>?> getEvents(
    ...
    final response = await http.get(
        Uri.parse(serverURL! +
            ...
            "&to=" +
            to),
        headers: headers).timeout(Duration(seconds: 90));
    ...
    if (response.statusCode == 200) {
      ...
      // Sort the list in descending order based on the 'timestamp' property
  ...
  static Future<http.Response> addNotification(String notif) async {
    ...
    /// For more fine-grained control over the request, use [Request] or
    /// [StreamedRequest] instead.
    final response = await http.post(Uri.parse(serverURL! + "/api/notifications"),
        body: notif, headers: headers);
    return response;
  }
}
